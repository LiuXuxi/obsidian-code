...
1. 数据设计
	...
	- 类的设计
		...
		protected:
			...
			IMAGE image;//植物贴图
			double init_time;//记录植物创建时间，以便计算行为时间
		};
...
2. 逻辑描述
	...
	2. 游戏开始后，环境根据计数时间开始产生阳关，僵尸根据出现时间了陆续出现在地图中，玩家可根据阳光数量进行植物创建，若创建位置合法，则创建，反之，不创建

	3. 僵尸行为：isOnMap=true，并以move_speed的移速开始沿y方向进行移动，当遇到植物时，即该位置的植物数组的指针非空时，僵尸开始以damage_speed的攻速和dagame的伤害开始攻击植物，直到僵尸死亡或植物死亡。若僵尸血量<=0，则死亡，isAlive=false，同时刷新image。

	4. 阳光行为：环境根据run_time产生阳光对象，阳光随机从x=0方向位置掉落到指定位置，向日葵根据创建对象的时间和产生阳光的速度创建阳光对象。阳光根据live_begin和livetime对比SUN_SPEED_LIVE判断阳光是否还存在，若不存在，刷新image

	5. 子弹行为：子弹为激活，isAlive=true，子弹初始位置为射手位置，以一定速度进行移动，直到遇到僵尸或者位置越界。遇到僵尸，对僵尸造成伤害，位置越界，无影响。

	6. 植物行为：当植物blood为0时，isAlive=false，刷新image
		1. 向日葵：据创建对象的时间和产生阳光的速度创建阳光对象

		2. 射手：根据init_time和damage_speed开始创建子弹对象，子弹对象激活

		3. 坚果：位置不变，直到血量为0，刷新image

	7. 碰撞检测：
		• 子弹与僵尸：同一行且距离小于阈值 → 僵尸扣血，子弹消失
		 • 僵尸与植物：位置重合 → 僵尸停止前进，持续对植物造成伤害 
		• 植物死亡：health ≤ 0 → 对应格子清空 • 僵尸死亡：health ≤ 0 → active = 0，不再参与逻辑

	8. 胜利判断：死亡僵尸数量等于初始僵尸数量

	9. 失败判断：存在僵尸位置小于等于地图最左侧