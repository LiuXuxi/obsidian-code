...
1. <font color=greeen>代码块的编译：</font>即使`constexpr if`的条件不满足，条件内的代码块仍会被编译。这意味着，如果<font color=red>条件分支内包含无效代码（如未定义的函数调用）</font>，编译器仍会报错。例如：
```
if constexpr (sizeof...(otherargs) > 100)
{
    testfunc();  // 如果 testfunc() 未定义，编译会报错
}
```
	 尽管`sizeof...(otherargs)`显然不可能大于100，编译器依然会检查`testfunc()`的定义。如果该函数未定义，编译会失败。
    
2. <font color=greeen>条件必须是常量：</font>`constexpr if`的条件必须在编译期确定。因此，不能使用在运行时才能确定的变量。例如，以下代码将会导致编译错误：
```
int i = 8;
if constexpr (i > 0)
{
...
```
	这里，`i`是一个普通变量，其值在运行时确定，因此不能用作`constexpr if`的条件。
