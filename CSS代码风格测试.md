...
当 CPU 给出一个虚拟地址（例如 `0x00007f8b4c23a000`）时，MMU 首先会把这个长串地址拆解成若干个“路标”和一个“偏移量”1。

想象你要去一个巨大的写字楼群找人，虚拟地址就像是一张复杂的寻人条：

1. **PGD Index (顶级目录索引)**：决定去哪个园区。
2. **PUD Index (上级目录索引)**：决定去园区里的哪栋楼。
3. **PMD Index (中间目录索引)**：决定去楼里的哪个楼层。
4. **PTE Index (页表索引)**：决定去楼层里的哪个房间号。
5. **Offset (页内偏移)**：决定在房间里具体拿哪一份文件。

### 🚶 第二步：逐级“翻页”查找

这个过程需要访问 4 次物理内存（每次去查一级表）：8

1. **找顶级页表 (PGD - Page Global Directory)**：
    
    - MMU 从 CPU 的 **CR3 寄存器**（或 ARM 的 TTBR）中取出**顶级页表的基地址**（这是进程页表的“总入口”）8。
    - 利用虚拟地址中的 **PGD Index**，在顶级页表里定位到具体的条目（PTE）。
    - **结果**：拿到**二级页表（PUD）的物理地址**。
2. **找二级页表 (PUD - Page Upper Directory)**：
    
    - MMU 拿到刚才得到的 PUD 表物理地址，结合虚拟地址中的 **PUD Index**，去查找二级页表。
    - **结果**：拿到**三级页表（PMD）的物理地址**。
3. **找三级页表 (PMD - Page Middle Directory)**：
    
    - MMU 拿到 PMD 表物理地址，结合虚拟地址中的 **PMD Index**，去查找三级页表。
    - **结果**：拿到**四级页表（PTE）的物理地址**。
4. **找最终页表 (PTE - Page Table Entry)**：
    
    - MMU 拿到 PTE 表物理地址，结合虚拟地址中的 **PTE Index**，终于查到了最终的页表项。
    - **结果**：拿到**物理页框号（PFN）**，也就是数据真正所在的“房间号”8。

### 📬 第三步：拼接与返回

拿到物理页框号后，MMU 还不能直接交给缓存，它需要把“房间号”和“文件位置”拼起来：

- 将 **物理页框号 (PFN)** 与虚拟地址末尾的 **Offset (页内偏移)** 拼接。
- 得到最终的 **物理地址**，然后去访问真正的物理内存，读取数据8。