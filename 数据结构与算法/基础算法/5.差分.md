...
[3453. 分割正方形 I - 力扣（LeetCode）](https://leetcode.cn/problems/separate-squares-i/solutions/3076424/zheng-shu-er-fen-pythonjavacgo-by-endles-8yn5/)
> 二维转一维
```cpp
class Solution {
public:
...
[995. K 连续位的最小翻转次数 - 力扣（LeetCode）](https://leetcode.cn/problems/minimum-number-of-k-consecutive-bit-flips/solutions/607416/k-lian-xu-wei-de-zui-xiao-fan-zhuan-ci-s-bikk/)

> 差分维护反转次数
> 差分的重点在于区间累加，考虑区间什么特点是不变的就维护什么。
> 有时并不是先计算完差分再处理，而是一边差分一边处理（同时预处理和处理）
```cpp
class Solution {
public:
...
[1526. 形成目标数组的子数组最少增加次数 - 力扣（LeetCode）](https://leetcode.cn/problems/minimum-number-of-increments-on-subarrays-to-form-a-target-array/submissions/718136219/)
> 类似于爬坡问题
> 统计爬坡高度
```cpp
class Solution {
...
[1943. 描述绘画结果 - 力扣（LeetCode）](https://leetcode.cn/problems/describe-the-painting/submissions/718459869/)
> 离散化坐标
> 当坐标值比较大时，直接用数组模拟坐标系会导致内存空间太大，此时需要将坐标离散化，即只记录状态改变或有特殊含义的坐标点
...
        for (auto const& [curr, d] : diff) {

            // 如果之前有颜色（sum > 0），则说明 [prev, curr] 是一个有效区间

            if (prev != -1 && sum > 0) {

                ans.push_back({(long long)prev, (long long)curr, sum});
...
## 2.二维差分
[2132. 用邮票贴满网格图 - 力扣（LeetCode）](https://leetcode.cn/problems/stamping-the-grid/submissions/718911604/)
> 差分和前缀和结合：就是多次前缀和
...
        for(int i2=stampHeight;i2<=n;i2++){

            for(int j2=stampWidth;j2<=m;j2++){

                int i1=i2-stampHeight+1;

                int j1=j2-stampWidth+1;

                if((preSum[i1-1][j1-1]+preSum[i2][j2]-preSum[i1-1][j2]-preSum[i2][j1-1])==0){
