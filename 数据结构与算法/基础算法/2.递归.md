...
            if (isalpha(s[i])) { // 使用 isalpha 简化判断

                res += s[i++];

            }

            else if (isdigit(s[i])) { // 使用 isdigit 简化判断
...
                // 修复：递归调用时，索引应该从 '[' 的下一个位置开始

                string temp = f(s, i + 1);

                res += build(cnt, temp);

                i = where + 1; // 从 ']' 的下一个位置继续
...
    string build(int cnt, string str) {

        string ans;

        // 如果 cnt 为 0（例如 "abc[d]"），则默认为 1 次
...
        map<string,int>ans=f(formula, 0);
...
            if (value > 1) build.append(to_string(value));
...
            else if (islower(formula[i]))name.push_back(formula[i++]);

            else cnt = cnt * 10 + formula[i++] - '0';
...
    void fill(map<string,int>&ans, map<string,int>&pre,string name,int cnt) {

        if (!pre.empty() || name.size()>0) {
...
        push(nums, characters, cur, '+');

        where = i;

        return static_cast<int>(compute(nums, characters));

    }

  

    void push(vector<long long>& nums, vector<char>& characters,long long cur,char c) {

        int n = nums.size();

        if (n == 0 || characters[n - 1] == '+' || characters[n - 1] == '-') {
...
            char character = characters[n - 1];

            if (character == '*')nums[n - 1] = num * cur;

            else nums[n - 1] = num / cur;
...
    long long compute(vector<long long>& nums, vector<char>& characters) {

        long long res = nums[0];

        for (int i = 1; i < nums.size(); i++) {

            res += characters[i - 1] == '+' ? nums[i] : -nums[i];
...
[0红黑树 - 蓝桥云课](https://www.lanqiao.cn/problems/20537/learning/)
> 向上递归和回溯树
```cpp
#include <iostream>
