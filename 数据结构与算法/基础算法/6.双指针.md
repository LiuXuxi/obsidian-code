...
##### [ 三数之和](https://leetcode.cn/problems/3sum/submissions/686012962/?envType=problem-list-v2&envId=two-pointers)
...
##### [最接近的三数之和](https://leetcode.cn/problems/3sum-closest/submissions/686034563/?envType=problem-list-v2&envId=two-pointers)
...
##### [ 四数之和](https://leetcode.cn/problems/4sum/?envType=problem-list-v2&envId=two-pointers)
...
#### [ 最长回文子串 ](https://leetcode.cn/problems/longest-palindromic-substring/submissions/685864282/?envType=problem-list-v2&envId=two-pointers)
...
> 仍然是中心拓展法。但是当第一次拓展到`s[l]!=s[r]`，因为有一次删除的机会，所以不能直接记录答案，而是使用条件（要么删左边，要么删右边），继续向两边拓展，最后再更新答案。
...
#### [3679. 使库存平衡的最少丢弃次数 - 力扣（LeetCode）](https://leetcode.cn/problems/minimum-discards-to-balance-inventory/submissions/702352434/)

> **数组中每一个相同的元素虽然数值相同，但是含义可能并不一样。** 当两者含义时不能视为一同，应该思考根据元素特性处理相同元素时的方式的差异。

1,2,3,3,3,4

虽然有多个相同的3号物品，但是每一个物品并不一样，一个物品不能被丢弃两次。那么出现了两个问题：

> 1.当相同类型物品数量超过m，是丢弃当前的物品，还是丢弃窗口内另一个物品？

==当天丢弃当前的物品==。因为==无论丢弃哪一个都至少需要丢弃一次，当之后的天数没有该类物品时，丢弃次数相同==；但如果之后的天数还有该类物品时，由于窗口会逐渐忽略更早到的物品（忽略相当于丢弃一次），选择丢弃越晚到的物品会越可能减少该类物品的数量（更可能达到丢弃一次和忽略一次的情况），更容易满足要求。

> 2.如何防止一个物品被丢弃两次（物品当天到达时与出窗口时）？

==丢弃该物品时，将`arrivals[i]`设为负数或者0== 。当该物品被丢弃时，等同于变为了一个不存在的类型的物品，由于`arrivals[i]>=1`，那么设为负数或者0 时既可以表示该物品被抛弃过，也不会对其他类型的物品的数量产生影响。
...
#### [每个字符最多出现两次的最长子字符串 - 力扣（LeetCode）](https://leetcode.cn/problems/maximum-length-substring-with-two-occurrences/)
...
- **指针移动策略**：
    ...
    2. **收缩**：若 `cnt[nums[R]]` 达到 $3$，$L$ 必须右移并递减经过元素的计数。
        
    3. **停止**：直到 `cnt[s[R]]` 恢复为 $2$。
...
            while(l<=r&&(cnt1>k||s[l]=='0'))cnt1-=s[l++]-'0';
...
#### [K 个不同整数的子数组 ](https://leetcode.cn/problems/subarrays-with-k-different-integers/description/)
...
#### [2762. 不间断子数组 - 力扣（LeetCode）](https://leetcode.cn/problems/continuous-subarrays/)
```cpp
class Solution {
...
            while(l<=r&&(cnt.rbegin()->first-cnt.begin()->first)>2){
...
#### [1358. 包含所有三种字符的子字符串数目 - 力扣（LeetCode）](https://leetcode.cn/problems/number-of-substrings-containing-all-three-characters/)
```cpp
class Solution {
...
#### [3298. 统计重新排列后包含另一个字符串的子字符串数目 II - 力扣（LeetCode）](https://leetcode.cn/problems/count-substrings-that-can-be-rearranged-to-contain-a-string-ii/)
```cpp
class Solution {
...
            if(++cnt[word1[r]-'a']==0)--debt;

            while(l<=r&&debt==0){

                if(--cnt[word1[l]-'a']==-1)++debt;
