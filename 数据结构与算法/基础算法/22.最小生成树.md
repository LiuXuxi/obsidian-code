...
int solve(){
    ...
    hash.insert(1);
    int ans=0;
    for(auto&edge:graph[1])heap.push({edge.first,edge.second});
    while(!heap.empty()){
        vector<int>cur=heap.top();
        heap.pop();
        int to=cur[0],cost=cur[1];
        auto it=hash.find(to);
        if(it==hash.end()){
            hash.insert(to);
            ans+=cost;
            for(auto&edge:graph[to])heap.push({edge.first,edge.second});
        }
    }
    return hash.size()==n?ans:-1;
}
