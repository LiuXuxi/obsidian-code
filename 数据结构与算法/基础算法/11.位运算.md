...
**记录已放置的皇后：** 每行都只能够放置一个皇后，用一个数组`line`来存储皇后的位置，索引`i`表示皇后的行数，`line[i]`表示皇后的列数。

**检查逻辑：** 检查该位置是否能够放置皇后的依据是==是否会与已放置的皇后产生冲突==，因此我们选择枚举皇后`line`，并判断各个方向是否会与该位置产生冲突，如果会，返回`false`，枚举完毕则表示不会，返回`true`。

1. **水平方向**：每个皇后都只会与自己所在行攻击，即`0~i-1`，不会影响`i`，无需判断
2. **垂直方向：**
