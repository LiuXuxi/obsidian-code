...
**记录已放置的皇后：** 每行都只能够放置一个皇后，用一个数组`line`来存储皇后的位置，索引`i`表示皇后的==行数==，`line[i]`表示皇后的==列数==。

**检查逻辑：** 检查该位置是否能够放置皇后的依据是==是否会与已放置的皇后产生冲突==，因此我们选择枚举皇后`line`，并==判断各个方向是否会与该位置产生冲突==，如果会，返回`false`，枚举完毕则表示不会，返回`true`。

1. **水平方向：** 每个皇后都只会与自己所在行攻击，即`0~row-1`，不会影响`i`，==无需判断==
2. **垂直方向：** 判断==该位置所列与之前皇后所列是否重合==，即`j==line[i]`，重合则会产生冲突，反之，则不会
3. **斜方向：** 学过高中数学的都知道，表格里的对角线的斜率的绝对值都为`1`，利用这个性质，我们可以直接用数学计算来判断，该位置的坐标可以表示`(row,j)`，已放置的皇后的坐标`(i,line[i])`，那么==依据斜率公式可以直接得到判别表达式==`j == line[k] || abs(k - i) == abs(line[k] - j`（*位置坐标不重要，斜率表达式才是关键*）
...
**大致思路：** 与常规解法相似，但是在空间上效率有大幅提高，依旧要枚举每一行每个位置。

**记录已放置的皇后：** 上一个解法用数组存储皇后所占列，这里依据位图用`bit`存储皇后已占用的列，由于`n`较小，用一个整数内存就够用

**检查逻辑：** 不依次枚举皇后是否产生冲突，而是==位运算一次性判断所有冲突==。用`candidate`得到==当前行所有可以放皇后的合法位置==
...
| `left`  | 被左斜线攻击的位置      |
| `right` | 被右斜线攻击的位置      |
| `limit` | 低 n 位为 1，其余为 0 |
`candidate = limit & (~(line | left | right));`：在二进制中`1`表示被攻击，`0`表示不被攻击，`line | left | right` 会统一所有`1`，得到的是==被受到其他皇后攻击的位置==（不可再放置皇后），对它进行取反操作即得到可以==能够再放置皇后的位置==

**掩码限制范围：** 我们并不需要用到位图中所有`bit`，只需要得到`0~n-1`位置上的信息，`limit = (1 << n) - 1`表示低 `n` 位全是 `1`，其他值（如`line left right`）和它`&`则高位全为`0`，自动==消除高位信息==，形成遮掩。
...
`candidate = 101100 -candidate = 010100 place = 000100`
...
[78. 子集 - 力扣（LeetCode）](https://leetcode.cn/problems/subsets/?envType=problem-list-v2&envId=bit-manipulation)
...
public:
    ...
    void f(vector<int>& nums, vector<vector<int>>&ans, vector<int>&path, int start) {
        ans.push_back(path);
    ...
    vector<vector<int>> subsets(vector<int>& nums) {
        ...
        for (int mask = 0; mask < total; mask++) {
            ...
            for (int j = 0; j < nums.size(); j++) {
                if (mask & (1 << j))path.push_back(nums[j]);
            }
            ans.push_back(path);
        }
        return ans;
...
        for(int i=__bit_width((unsigned int)mx)-1;i>=0;i--){

            //掩码分为两个部分：

            //高于等于i位的位数：和ans一样，表示确定（期盼）的高位

            //低于i位的位数：全为1，表示不关心低位（无论为1还是0）
...
            //每一行都至少有一个数的第i位为0，若某一行的第i位都为1，无论怎样或运算第i位都为1，因为1|0=1，1|1=1，0|1=1

            //不会影响已经确定好的高位（高于i的位数，也就是ans的高位）

            for(auto&nums:grid){

                //若当前行的第i位全为1，则false保持不变

                bool found0=false;

                for(auto&num:nums){

                    //条件为真：当前num不仅第i为0，而且不影响ans的高位，直接跳过该行
