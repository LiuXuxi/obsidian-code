...
            vector<int>pos=wait.top();

            wait.pop();

            int x=pos[0],y=pos[1],w=pos[2];
...
                    wait.push({nx,ny,max(heightMap[nx][ny],w)});
...
![[Pasted image 20260210181919.png]]

[127. 单词接龙 - 力扣（LeetCode）](https://leetcode.cn/problems/word-ladder/description/)

[P1135 奇怪的电梯 - 洛谷](https://www.luogu.com.cn/problem/P1135#ide)
...
int n,a,b;
int arr[205];
vector<int>dis(205,INT_MAX);

void dfs(int i,int steps){
    dis[i]=steps;
    int v=i-arr[i];
    if(v>=1&&steps+1<dis[v])dfs(v,steps+1);
    v=i+arr[i];
    if(v<=n&&steps+1<dis[v])dfs(v,steps+1);
}
...
int main(){
    ...
    dfs(a,0);
    cout<<(dis[b]==INT_MAX?-1:dis[b])<<endl;
    return 0;
}
```