...
[1385. 两个数组间的距离值 - 力扣（LeetCode）](https://leetcode.cn/problems/find-the-distance-value-between-two-arrays/)
```cpp
class Solution {
...
**单调性：** 矩阵上任何一点的==右边和下边的值都更大，左边和上边的值都更小==。搜索要根据单调性进行特定的搜索策略，所有我们可以选择==一边大一边小==的搜索策略进行搜索，而如果选择两大或者两小的话搜索就会有==歧义==。

**搜索起点：** 
> ==矩阵的右上角：==
> 该点左边的值都更小，该点下边的值都更大。*这里我们选择的就是该点。*
...
- 以`row`记录搜索点的<font color=greeen>行坐标</font>
- 以`line`记录搜索点的<font color=greeen>列坐标</font>
- 当`matrix[row][line]==target`时，说明`target`在矩阵内，返回`true`

>以右上角为搜索起点

- 当`target>matrix`，==向上移动==，`++row`
- - 当`target<matrix`，==向左移动==，`--line`

> 以左下角为搜索起点

- 当`target>matrix`，==向右移动==，`++line`
- 当`target<matrix`，==向上移动==，`--row`
...
            else if (target>matrix[row][line])++line;
...
**单调性：** 每一行都是==升序的一维数组==，具有单调递增的特性，因此我们可以对每一行都进行数值查找的二分策略。

**搜索策略：** ==对每一行进行二分搜索==

- `matrix[mid]==target`，返回`true`
- `matrix[mid]>target`，==向左继续搜索==，`right=mid-1`
- `matrix[mid]<target`，==向右继续搜素==，`left=mid+1`
...
        auto it=cnt.find(value);

        if(it==cnt.end())return 0;

        vector<int>&nums=it->second;

        return upper_bound(nums.begin(),nums.end(),right)-lower_bound(nums.begin(),nums.end(),left);
...
[3488. 距离最小相等元素查询 - 力扣（LeetCode）](https://leetcode.cn/problems/closest-equal-element-queries/submissions/704364243/)
```cpp
class Solution {
...
            int j=lower_bound(arr.begin(),arr.end(),query)-arr.begin();

            ans.push_back(min(arr[j+1]-query,query-arr[j-1]));
...
[1146. 快照数组 - 力扣（LeetCode）](https://leetcode.cn/problems/snapshot-array/)
```cpp
class SnapshotArray {
...
        vector<pair<int, int>>& arr = history[index];

        // 查找第一个 snap_id > 查询 snap_id 的位置

        auto it = upper_bound(arr.begin(), arr.end(), make_pair(snap_id, INT_MAX));

        // 如果是第一个位置，说明之前没有记录

        if (it == arr.begin()) return 0;

        // 返回前一个位置的值（即 snap_id <= 查询 snap_id 的最大记录）
...
> 与数学公式结合的题型，如$gcd$和$lcm$

[[12.最大公约数和同余原理#[878. 第 N 个神奇数字 - 力扣（LeetCode）](https //leetcode.cn/problems/nth-magical-number/)]]
...
        if(a[i]*time>b[i])sum+=a[i]*time-b[i];
...
    for(double l=0,r=1e10,m;r-l>1e-6;){
...
>==二分不是在找答案==，而是在有序的‘备选池’里寻找当前元素最合适的坑位。核心是利用二分保持‘备选池’的有序性。

####  **1.1 标准 LIS 问题**：基础 $O(n \log n)$ 模板。
 [300. 最长递增子序列 - 力扣（LeetCode）](https://leetcode.cn/problems/longest-increasing-subsequence/?envType=problem-list-v2&envId=binary-search)    
...
```
#### **1.2 二维/多维 LIS**：如“俄罗斯套娃信封”，需先排序消元，再降维成 LIS。
[354. 俄罗斯套娃信封问题 - 力扣（LeetCode）](https://leetcode.cn/problems/russian-doll-envelopes/?envType=problem-list-v2&envId=binary-search)                                  
...
```
####  **1.3 最小列覆盖（狄尔沃斯定理）**：将问题转化成求 LIS 长度，例如“导弹拦截”问题。
    

### 2. 单调结构维护 (Monotonic Structure)      

二分不只是为了找答案，而是为了在不断变化的序列中快速找到插入位置。

#### **2.1 动态维护中位数**：利用双堆或平衡树思想，配合二分定位。
    
#### **2.2 插入排序的优化**：在维护有序列表时，用二分查找插入位置以减少比较次数（虽然位移仍是 $O(n)$）。
    

### 3. 数据范围缩减与判定 (Data Filtering)

#### **3.1 寻找第 K 小/大的数值**：例如“两个有序数组的中位数”或“乘法表中第 K 小的数”。这类题虽然像二分答案，但本质是在维护/统计区间。
    
#### **3.2 限制条件下的最优配对**：在有序数组中，利用二分快速找满足条件 `nums[i] + nums[j] < target` 的边界（如三数之和、四数之和的内部优化）。

#### **3.3最小化极差** 
[U535992 J-C 小梦的宝石收集 - 洛谷](https://www.luogu.com.cn/problem/U535992#ide)
```cpp
#include<iostream>
...
            if((i-1+n-m+min(i-1,n-m))<=k)r=m;

            else l=m+1;

        }

        ans=min(ans,arr[l]-arr[i]);
...
### 4. 几何与空间二分 (Geometry & Range)

####  **4.1 最近点对问题**：在分治算法中，利用二分在 $Y$ 轴有序数组中快速筛选候选点。
    
####  **4.2 覆盖区间统计**：给定一组区间，利用二分查找快速判定新区间与已有区间的重叠情况。
    

### 5.结合预处理

[2389. 和有限的最长子序列 - 力扣（LeetCode）](https://leetcode.cn/problems/longest-subsequence-with-limited-sum/submissions/704213707/)
...
        for(auto&query:queries)ans.push_back(upper_bound(pre.begin(),pre.end(),query)-pre.begin()-1);
...
[2070. 每一个查询的最大美丽值 - 力扣（LeetCode）](https://leetcode.cn/problems/most-beautiful-item-for-each-query/)
```cpp
class Solution {
...
        int n=items.size(),maxV=0;

        for(int i=0;i<n-1;i++){

            maxV=max(maxV,items[i][1]);
