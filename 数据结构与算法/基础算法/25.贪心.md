...
[记录详情 - 洛谷 | 计算机科学教育新生态](https://www.luogu.com.cn/record/264070069)
```cpp
#include<iostream>
#include<vector>
#include<algorithm>
...
int solve(){
    ...
    if(n>=1)dp[0]=times[0];
    if(n>=2)dp[1]=times[1];
    if(n>=3)dp[2]=times[0]+times[1]+times[2];
    for(int i=3;i<n;i++){
        dp[i]=min(times[i]+times[0]+dp[i-1],
        dp[i-2]+times[1]+times[i]+times[0]+times[1]);
    }
    return dp[n-1];
}
...
[517. 超级洗衣机 - 力扣（LeetCode）](https://leetcode.cn/problems/super-washing-machines/)
```cpp
class Solution {
...
        int n=machines.size(),sum=0,ans=0;
...
            rightNeed=(n-1-i)*avg-(sum-leftSum-machines[i]);

            if(leftNeed>0&&rightNeed>0)bottleNeck=leftNeed+rightNeed;

            else bottleNeck=(int)max(fabs(leftNeed),fabs(rightNeed));
...
[1921. 消灭怪物的最大数量 - 力扣（LeetCode）](https://leetcode.cn/problems/eliminate-maximum-number-of-monsters/)
```cpp
class Solution {
...
[记录详情 - 洛谷 | 计算机科学教育新生态](https://www.luogu.com.cn/record/268770080)
```cpp
#include<iostream>
#include<string.h>
using namespace std;
...
bool solve(int cnt,int cur){
    if(cnt>=n)return 1;
    for(int i=1;i<=n;i++){
        if(bk[i]||arr[i][0]+arr[i][1]<cur)continue;
        bk[i]=1;
        if(solve(cnt+1,max(arr[i][0],cur)+arr[i][2])){
            bk[i]=0;
            return 1;
        }
        bk[i]=0;
...
[P10387 [蓝桥杯 2024 省 A] 训练士兵 - 洛谷](https://www.luogu.com.cn/problem/P10387#ide)
...
[5.机器人充电【算法赛】 - 蓝桥云课](https://www.lanqiao.cn/problems/21334/learning/?contest_id=296)
> 充电问题：贪心+排序
> 选择完成工作的时间最小的
...
long long sum_base=0,cur=0,min_finish;
...
    cur=arr[0][0],min_finish=arr[0][0]+arr[0][1];
...
    long long ans=min_finish-sum_base;
...
假设有一组数据点 $x_1, x_2, \dots, x_n$，每个点都有一个对应的权重 $w_1, w_2, \dots, w_n$。

**加权中位数** 是指一个值 $x_k$，它能将总权重几乎平分为两半。

具体数学定义为，找到一个 $x_k$，使得：

- 所有小于 $x_k$ 的点的权重之和 $\le$ 总权重的一半。
    
- 所有大于 $x_k$ 的点的权重之和 $\le$ 总权重的一半。
...
在处理**最小化代价**问题时，它是终极武器。

- **普通中位数**：最小化 $\sum |x_i - x|$（所有点到 $x$ 的距离之和）。
    
- **加权中位数**：最小化 $\sum w_i \cdot |x_i - x|$（所有点到 $x$ 的**带权**距离之和）。
    

**生活实例：**

假设你要建一个物流中心。有 5 个城市需要送货，但每个城市的进货量（权重）不同。为了让总运费（距离 $\times$ 货物量）最低，物流中心应该建在这些城市的**加权中位数**位置，而不是地理中心。

---

 **3. 如何计算加权中位数？（步骤详解）** 

求加权中位数的标准算法复杂度通常是 $O(n \log n)$，步骤如下：

 第一步：排序

	将所有数据点 $x_i$ 按照数值大小进行**升序排列**。注意：权重 $w_i$ 必须跟着相应的数据点一起移动。

第二步：求总权重

	计算所有权重的总和 $W = \sum_{i=1}^{n} w_i$。

第三步：累加查找

	从最小的数据点开始，逐个累加权重。当你累加到某一个点 $x_k$，使得当前的累计权重 $S_k \ge \frac{W}{2}$ 时，这个 $x_k$ 就是加权中位数。


#### 应用

[6.统一积木的最小代价【算法赛】 - 蓝桥云课](https://www.lanqiao.cn/problems/21336/learning/?contest_id=296)
```cpp
#include <iostream>
...
    for(int i=0;i<n;i++)ans+=1LL*abs(arr[i][0]-avg)*arr[i][1];
