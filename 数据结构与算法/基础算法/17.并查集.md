...
[928. 尽量减少恶意软件的传播 II - 力扣（LeetCode）](https://leetcode.cn/problems/minimize-malware-spread-ii/submissions/696985075/)

```cpp
class Solution {
public:
    int minMalwareSpread(vector<vector<int>>& graph, vector<int>& initial) {
        build(graph.size(), initial);
        for (int i = 0; i < graph.size(); i++) {
            for (int j = 0; j < graph.size(); j++) {
                if (graph[i][j] == 1 && !virus[i] && !virus[j])Union(i, j);
            }
        }
        for (auto& viru : initial) {
            for (int neighbor = 0; neighbor < graph.size(); neighbor++) {
                if (!virus[neighbor] && neighbor!=viru&&graph[viru][neighbor]) {
                    int fn = find(neighbor);
                    if (infect[fn] == -1)infect[fn] = viru;
                    else if (infect[fn] != -2 && infect[fn] != viru)infect[fn] = -2;
                }
            }
        }
        for (int i = 0; i < graph.size(); i++) {
            if (i == find(i) && infect[i] >= 0)cnt[infect[i]] += size[i];
        }
        sort(initial.begin(), initial.end(), [](int a, int b) {return a < b; });
        int ans = initial[0];
        for (auto& viru : initial) {
            if (cnt[viru] > cnt[ans])ans = viru;
        }
        return ans;
    }
...
private:
    ...
    void build(int n,vector<int>&initial) {
        ...
        for (auto& viru : initial)virus[viru] = true;
        cnt.resize(n, 1);
    }
